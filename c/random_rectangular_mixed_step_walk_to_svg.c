 ...
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Perform a random walk with steps in the set {North, South, East, West}*
 *      but where the same direction is not allowed to be repeated twice in   *
 *      a row. So if North-South occurs the first time, the second step must  *
...
/*  Function for computing a random North-South walk.                         */
static struct pair random_updown(void)
{
    /*  Get a random real number between 0 and 2 using stdlib.h.              */
    double rreal = (double)rand() / (double)RAND_MAX;

    /*  Convert this to an integer, either 0, 1, or 2.                        */
    int rint = (int)(2.0 * rreal);
    struct pair out;

    /*  0 has a 49.9999998% probability, 1 has a 49.9999998%, and 2 has a     *
     *  0.000000004% probability of occuring. Since 2 is so unlikely, assume  *
     *  it's impossible and just group 2 together with 1. So, the             *
     *  possibilities are rint = 0, and rint != 0 with ~50% probability each. */
    if (rint == 0)
    {
        out.x = 0.0;
        out.y = 1.0;
...
/*  End of random_leftright.                                                  */

/*  Function for computing a random East-West walk.                           */
static struct pair random_leftright(void)
{
    /*  Use the same method as random_updown, but with steps in (1, 0) and    *
     *  (-1, 0), instead of (0, 1), (0, -1).                                  */
    double rreal = (double)rand() / (double)RAND_MAX;
    int rint = (int)(2.0 * rreal);
    struct pair out;
...
int main(void)
{
    ...
    FILE *fp = fopen("tmpl_random_walk_rectangular_mixed.svg", "w");
    ...
    for (n = 1U; n < walk_size; ++n)
    {
        ...
        /*  For odd n, use North-South steps, and for even East-West. We can  *
         *  check if n is even with a bit-wise &. If n is odd, in binary, the *
         *  last digit is 1. n & 0x01 zeroes out every digit of n except the  *
         *  last one. So, if n & 0x01 is 1, n is odd. If n & 0x01 is 0, n is  *
         *  even.                                                             */
        if (n & 0x01)
            A[n] = add_pairs(A[n-1], random_updown());
        else
            A[n] = add_pairs(A[n-1], random_leftright());
